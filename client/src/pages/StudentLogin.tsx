...
import { loginStudentOnline } from "@/store";
...
export default function StudentLogin({ onSuccess, onBack }: Props) {
  ...
  const handleLogin = async (event?: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      onSuccess(await loginStudentOnline(email, password));
    } catch (cause) {
  ...
  return <main className="min-h-screen bg-[#f8fafc] px-4 py-5 sm:px-8 sm:py-8"><div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-5xl items-center"><div className="w-full max-w-md"><button onClick={onBack} className="mb-8 inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-slate-900"><ArrowLeft className="h-4 w-4" /> Voltar ao início</button><div className="soft-panel rounded-3xl border-2 border-indigo-100 bg-white/95 p-5 shadow-[0_8px_0_#e0e7ff] sm:p-9"><div className="mb-8 flex items-center gap-3"><span role="img" aria-label="Livro" className="grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br from-indigo-500 to-fuchsia-500 text-3xl shadow-[0_5px_0_#c7d2fe]">📖</span><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-teal-600">Letter Play</p><h1 className="font-title text-3xl font-bold text-slate-950">Área do aluno</h1></div></div><p className="mb-7 text-sm leading-relaxed text-slate-500">Entre para continuar suas atividades em qualquer dispositivo.</p>{error && <div className="mb-4 border-l-2 border-rose-500 bg-rose-50 px-3 py-2 text-sm font-semibold text-rose-700">{error}</div>}<form onSubmit={handleLogin} className="space-y-5"><label className="block"><span className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-500">Email</span><input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-900 outline-none focus:border-teal-500" /></label><label className="block"><span className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-500">Senha</span><input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-900 outline-none focus:border-teal-500" /></label><button disabled={loading} type="submit" className="flex w-full items-center justify-center gap-2 rounded-2xl bg-teal-500 px-4 py-3.5 text-sm font-extrabold text-white shadow-[0_5px_0_#0f766e] hover:bg-teal-600 disabled:cursor-wait disabled:opacity-60 active:translate-y-1 active:shadow-none">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <LogIn className="h-4 w-4" />} {loading ? "Conectando..." : "Entrar no jogo"}</button></form><p className="mt-6 border-t border-slate-100 pt-4 text-xs text-slate-400">Os dados e o progresso são sincronizados online.</p></div></div></div></main>;
}
