          ...
          <div>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {GAME_KEYS.map((item) => {
                ...
                return <button key={item.key} onClick={() => onSelectGame(item.screen)} className="group soft-panel flex min-h-[148px] flex-col justify-between p-5 text-left hover:-translate-y-0.5 hover:border-teal-300">
                  <div><div className="mb-4 text-xs font-bold uppercase tracking-[0.14em] text-slate-400">Atividade</div><h2 className="font-title text-xl font-bold text-slate-900 group-hover:text-teal-700">{info.name}</h2><p className="mt-1 text-sm leading-snug text-slate-500">{info.desc.replace(/^[^a-zA-ZÀ-ÿ]*/, "")}</p></div>
